...
test('describes stake and claim intent from the live staking controls', () => {
    ...
    assert.match(claim.detail, /final claim amount when the transaction executes/);
});
...
test('describes bulk and individual unstake amounts', () => {
    ...
    assert.match(bulk.detail, /still locked are not included/);
