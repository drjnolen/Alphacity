...
test('nine bosses expose distinct protocols, fixed telegraphs, phases and meaningful effects',()=>{
 assert.equal(new Set(BOSS_PROTOCOLS.map(p=>p.name)).size,9);for(let ch=1;ch<=9;ch++){let r=boss(ch),c=r.combat,e=c.enemies[0];assert.ok(e.ability);e.charger=false;delete e.charge;const before=JSON.stringify(e.intent);e.hp=Math.floor(e.maxHp*.25);assert.equal(JSON.stringify(e.intent),before);planEnemies(c);assert.equal(c.enemies[0].phase,ch===9?3:2);c.enemies.forEach(n=>n.poison=0);r.hp=r.maxHp=500;const end=transition(r,{type:'end'}),cue=combatFeedback(r,end,{type:'end'});assert.equal(cue.bossEffect,BOSS_PROTOCOLS[ch-1].effect);assert.ok(cue.attacks.some(a=>a.boss&&a.effect));}
});
