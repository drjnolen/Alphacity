...
test('continuing districts retains health, supplies, loadout and upgrades; completing nine resets them',()=>{
 ...
 assert.equal(r.chapter,9);assert.equal(r.climbActive,false);assert.deepEqual(r.boosts,{});assert.equal(b.inventory.length,inventory);assert.equal(b.cleared.length,0);assert.ok(Object.values(b.xp).every(x=>x===0));assert.equal(b.salvage,0);
});
