import {CLOCKS} from './expedition.ts';
...
export function combatFeedback(before: Run, after: Run, action: GameAction): CombatCue | null {
  ...
  if(action.type==='overclock'){cue.title=CLOCKS[action.slot].name;cue.subtitle=action.slot.toUpperCase()+' · GEAR OVERCLOCK';cue.tone='gold';}
  ...
  if (action.type==='attack' && kind === 'signature' && before.heroId === 'glitchborn' && !cue.destination && cue.victory) {
    cue.destination = blinkLanding(c,target);
  }
  if(action.type==='attack'&&kind==='signature'&&before.heroId==='coinbroker')cue.subtitle='VEX · TARGET JAMMED';
  ...
  if(action.type==='overclock'){const e=after.combat?.enemies.find(e=>e.id===action.id);if(e&&action.slot==='cranial')cue.impacts.push({position:e,text:'EXPOSED +'+e.exposed,tone:'gold',delay:450});if(e&&action.slot==='tool')cue.impacts.push({position:e,text:'SLOWED · SUPPRESSED',tone:'gold',delay:450});if(e&&!same(e,target))cue.movements.push({source:target,target:e,portrait:enemyPortrait(before,e),delay:500,path:[target,e],enemyId:e.id});if(after.combat&&after.combat.block!==c.block)cue.impacts.push({position:c.hero,text:(after.combat.block>c.block?'+':'')+(after.combat.block-c.block)+' GUARD',tone:'gold',delay:250});}
  if(after.hp>before.hp&&kind!=='heal'&&kind!=='end')cue.impacts.push({position:c.hero,text:'+'+(after.hp-before.hp)+' HEALTH',tone:'heal',delay:600});
  if((action.type==='attack'||action.type==='overclock')&&after.salvage>before.salvage)cue.impacts.push({position:target,text:`+${after.salvage-before.salvage} CREDITS`,tone:'gold',delay:500});
  ...
  if (action.type==='attack' && kind === 'signature' && before.heroId === 'chainbreaker' && after.combat) cue.impacts.push({position:c.hero,text:`+${after.combat.block-c.block} GUARD`,tone:'gold',delay:500});
