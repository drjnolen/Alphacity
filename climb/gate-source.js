...
async function enter(){
 ...
 try{
  const result=await balances(address);if(requestEpoch!==epoch)return;balanceText(result);
...
async function refresh(){
 ...
 try{const result=await balances(address);if(requestEpoch!==epoch)return;balanceText(result);if(!result.allowed)lock('Your balance is now below 5,000,000 CITY. Restore your balance and verify to resume.');}
 catch{if(requestEpoch===epoch)lock('CITY access could not be rechecked. Verify again to resume your saved climb.');}
 finally{if(requestEpoch===epoch){checking=false;verify.disabled=!wallet;}}
}
try{
 if(!window.AlphaCityWalletConnector||!window.AlphaCitySui)throw Error('Wallet services failed to load. Refresh to retry.');
 connector=window.AlphaCityWalletConnector.create({button:el('connect-wallet'),requirePersonalMessage:true,onChange(session){epoch++;wallet=session;checking=false;lock(session?'Wallet connected. Verify access to begin.':'Connect your Sui wallet to play.');verify.disabled=!session;el('membership-status').textContent='5,000,000 CITY required';el('liquid-city').textContent='—';el('staked-city').textContent='—';}});
